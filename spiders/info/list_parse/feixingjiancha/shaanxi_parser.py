...
class ShaanXiInfoParser(BaseParser):
    """
    陕西省药品监督管理局  https://mpa.shaanxi.gov.cn/zfxxgk/fdzdgknr/wjtz.htm
    """

    def start_requests(self):
        url = "https://mpa.shaanxi.gov.cn/zfxxgk/fdzdgknr/wjtz.htm"
    ...
    def parse(self, request, response):
        ...
        title_contents = response.xpath("//table/tr")

        for title_content in title_contents[1:]:
            
            title_td = title_content.xpath("./td")
            title = title_td[0].xpath("./a/@title").extract_first()
            url = title_td[0].xpath("./a/@href").extract_first()
            ...
            item.release_time = title_td[1].xpath("./text()").extract_first()
            item.source = source
            item.title = title
