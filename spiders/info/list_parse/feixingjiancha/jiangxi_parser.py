...
class JiangXiInfoParser(BaseParser):
    """
    江西省药品监督管理局  http://mpa.jiangxi.gov.cn/col/col51884/index.html
    """

    def start_requests(self):
        url = "http://mpa.jiangxi.gov.cn/col/col51884/index.html"
    ...
    def parse_for_page(self, request, response):
        ...
        headers = {
            ...
            'Referer': 'http://mpa.jiangxi.gov.cn/col/col51884/index.html?uid=358871&pageNum=1',
        }
        from_data = {
            "col": "1",
            "webid": "201",
    ...
    def parse(self, request, response):
        ...
        source = "飞行检查.江西省药品监督管理局"
        save_count = 0
        filter_word1 = ["医疗器械", "飞行检查"]
        filter_word2 = ["公司", "厂", "停产整改", "恢复生产"]
        ...
        for title_content in title_contents[:45]:
            ...
            condj = [all(ext in title for ext in filter_word1), any(ext in title for ext in filter_word2[:2]) and any(ext in title for ext in filter_word2[2:])]
            if not condj[0]:
                if not condj[1]:
                    continue
                else:
