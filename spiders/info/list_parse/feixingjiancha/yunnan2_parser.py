...
class YunNan2InfoParser(BaseParser):
    """
    云南省药品监督管理局  http://mpa.yn.gov.cn/newsite/ZwgkNewsList.aspx?CID=03fddf74-e4f3-41c0-a11b-a626a789900e
    """

    def start_requests(self):
        url = "http://mpa.yn.gov.cn/newsite/ZwgkNewsList.aspx?CID=03fddf74-e4f3-41c0-a11b-a626a789900e"
        yield feapder.Request(url=url, callback=self.parse_for_page)

    def parse_for_page(self, request, response):
        
        page_num = int(re.findall("共(.*?)页", response.text)[0])
        base_url = "http://mpa.yn.gov.cn/newsite/ZwgkNewsList.aspx?CID=03fddf74-e4f3-41c0-a11b-a626a789900e&page={}"
    ...
    def parse(self, request, response):
        ...
        source = "飞行检查.云南省药品监督管理局"
        save_count = 0
        filter_word1 = ["医疗器械", "飞行检查"]
        filter_word2 = ["公司", "厂", "飞行检查"]
        title_contents = response.xpath("//ul[@class='zfxxgnewlist']/li")
...
if __name__ == "__main__":
    ...
    spider = tmp(redis_key="test_YunNan2InfoParser", delete_keys=True)
    spider.add_parser(YunNan2InfoParser)
