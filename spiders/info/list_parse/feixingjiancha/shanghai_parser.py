...
class ShangHaiInfoParser(BaseParser):
    """
    上海市药品监督管理局  https://yjj.sh.gov.cn/zx-ylqx/index.html
    """

    def start_requests(self):
        url = "https://yjj.sh.gov.cn/zx-ylqx/index.html"
        yield feapder.Request(url=url, callback=self.parse_for_page)

    def parse_for_page(self, request, response):
        
        page_num = int(re.findall("totalPage: (\d+)", response.text)[0])
        base_url = "https://yjj.sh.gov.cn/zx-ylqx/index_{}.html"
        # page_num = 2
    ...
    def parse(self, request, response):
        ...
        source = "飞行检查.上海市药品监督管理局"
        save_count = 0
        filter_word1 = ["医疗器械", "飞行检查"]
        filter_word2 = ["公司", "厂", "飞行检查"]
        title_contents = response.xpath("//ul[@class='uli14 pageList nowrapli borderli']//li")

        for title_content in title_contents:

            title = title_content.xpath("./a/text()").extract_first().strip()
            condj = any([all(ext in title for ext in filter_word1), any(ext in title for ext in filter_word2[:2]) and (filter_word2[2] in title)])
            if not condj:
                continue
            ...
            item.release_time = re.sub(r"\.", "-", title_content.xpath("./small/text()").extract_first())
            item.source = source
            item.title = title
