...
class GuangDongInfoParser(BaseParser):
    """
    广东省药品监督管理局  http://mpa.gd.gov.cn/zwgk/jgsz/ylqxaqjgc/fxjc/index.html
    """

    def start_requests(self):
        url = "http://mpa.gd.gov.cn/zwgk/jgsz/ylqxaqjgc/fxjc/index.html"
        yield feapder.Request(url=url, callback=self.parse_for_page)

    def parse_for_page(self, request, response):
        
        page_num = int(re.findall(r"index_(\d+)\.", response.xpath("//div[@id='page_div']/a[@class='last']/@href").extract_first())[0])
        base_url = "http://mpa.gd.gov.cn/zwgk/jgsz/ylqxaqjgc/fxjc/index_{}.html"
    ...
    def parse(self, request, response):
        ...
        source = "飞行检查.广东省药品监督管理局"
        save_count = 0

        title_contents = response.xpath("//ul[@class='infoList clearfix']/li")
        ...
        for title_content in title_contents:
            ...
            item.release_time = re.findall(r"(\d{4}-\d{2}-\d{2})", title_content.xpath(".//span/text()").extract_first())
            item.source = source
            item.title = title
...
if __name__ == "__main__":
    ...
    spider = tmp(redis_key="test_GuangDongInfoParser", delete_keys=True)
    spider.add_parser(GuangDongInfoParser)
