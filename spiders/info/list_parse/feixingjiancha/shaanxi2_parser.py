...
class ShaanXi2InfoParser(BaseParser):
    """
    陕西省药品监督管理局  https://mpa.shaanxi.gov.cn/listdt.jsp?urltype=tree.TreeTempUrl&wbtreeid=1176
    """

    def start_requests(self):
        url = "https://mpa.shaanxi.gov.cn/listdt.jsp?urltype=tree.TreeTempUrl&wbtreeid=1176"
        yield feapder.Request(url=url, callback=self.parse_for_page)

    def parse_for_page(self, request, response):
        
        page_num = int(re.findall(r'p=(\d+)&a249784c=18&urltype\=tree\.TreeTempUrl&wbtreeid\=1176">尾页', response.text)[0])
        base_url = "https://mpa.shaanxi.gov.cn/listdt.jsp?a249784t=6&a249784p={}&a249784c=18&urltype=tree.TreeTempUrl&wbtreeid=1176"
    ...
    def parse(self, request, response):
        ...
        for title_content in title_contents:
            ...
            if not(all(ext in title for ext in filter_word1)):
                continue
            # if not(any(ext in title for ext in filter_word2)):
            #     continue
...
if __name__ == "__main__":
    ...
    spider = tmp(redis_key="test_ShaanXi2InfoParser", delete_keys=True)
    spider.add_parser(ShaanXi2InfoParser)
