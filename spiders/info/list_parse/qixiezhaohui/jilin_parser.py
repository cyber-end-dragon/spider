...
class JiLinInfoParser(BaseParser):
    """
    吉林省药品监督管理局  http://mpa.jl.gov.cn/zxfw_84842/gsgg/pgzg/
    """

    def start_requests(self):
        url = "http://mpa.jl.gov.cn/zxfw_84842/gsgg/pgzg/"
        yield feapder.Request(url=url, callback=self.parse_for_page)

    def parse_for_page(self, request, response):
        
        page_num = int(re.findall("var pageNum = (\d+)", response.text)[0])
        base_url = "http://mpa.jl.gov.cn/zxfw_84842/gsgg/pgzg/index_{}.html"
    ...
    def parse(self, request, response):
        ...
        source = "器械召回.吉林省药品监督管理局"
        save_count = 0
        filter_word = ["医疗器械", "召回公示"]
        title_contents = response.xpath("//ul[@class='list sz_list allList']/li")


        for title_content in title_contents:

            title = title_content.xpath("./a/text()").extract_first().strip()
            if not(all(ext in title for ext in filter_word)):
                continue
...
if __name__ == "__main__":
    ...
    spider = tmp(redis_key="test_JinLinInfoParser", delete_keys=True)
    spider.add_parser(JiLinInfoParser)
