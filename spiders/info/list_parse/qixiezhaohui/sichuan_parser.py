...
class SiChuanInfoParser(BaseParser):
    """
    四川省药品监督管理局  http://yjj.sc.gov.cn/scyjj/c103165/cpzhylqx.shtml
    """

    def start_requests(self):
        url = "http://yjj.sc.gov.cn/scyjj/c103165/cpzhylqx.shtml"
        yield feapder.Request(url=url, callback=self.parse_for_page)

    def parse_for_page(self, request, response):
        
        page_num = int(re.findall("createPageHTML\('page_div',(\d+),", response.text)[0])
        base_url = "http://yjj.sc.gov.cn/scyjj/c103165/cpzhylqx_{}.shtml"
    ...
    def parse(self, request, response):
        ...
        source = "器械召回.四川省药品监督管理局"
        save_count = 0
        # filter_word1 = ["医疗器械", "飞行检查"]
        title_contents = response.xpath("//td[@class='ListColumnClass5']")
        time_contents = response.xpath("//td[@class='listtddate5']")

        for idx, title_content in enumerate(title_contents):
            
            title = title_content.xpath("./a/@title").extract_first()
            ...
            item.release_time =time_contents[idx].xpath("./text()").extract_first()
            item.source = source
            item.title = title
...
if __name__ == "__main__":
    ...
    spider = tmp(redis_key="test_SiChuanInfoParser", delete_keys=True)
    spider.add_parser(SiChuanInfoParser)
